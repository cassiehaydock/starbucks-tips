...
namespace StarbTips
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //create an array of the names of all employees read in from a file
            string path = @"employees.txt";
            string[] text = System.IO.File.ReadAllLines(path);
            string[] names = new string[text.Length];

            //split the file up based on newlines
            for (int i = 0; i < text.Length; i++)
            {
                string[] partial = text[i].Split('\n', StringSplitOptions.None);
                names[i] = partial[0].Trim();
            }
                    ...
                    case 1:
                        {
                            ...
                            //write the new employee name to file
